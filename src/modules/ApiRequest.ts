...
export const getMovieDetails = async ({ id }: GetMovieDetailsParams) => {
    console.log(id);
    const { data: movie } = await instance.get<GetMovieResponse>(
        `movie/${id}`, // GET /movie/{movie_id}/credits
        {
            params: {
    ...
    const releaseDate = (() => {
        console.log('release_dates', movie.release_dates);
